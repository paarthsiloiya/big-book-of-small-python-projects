...
COLORS_MAP = {0: 'red', 1: 'green', 2: 'white',
              3: 'yellow', 4: 'cyan', 5: 'magenta'}
...
def main():
    ...
    bext.fg('white')
    bext.clear()
    print('''FLOODER - The Color Flood Game

OBJECTIVE: Fill the entire board with a single color!

HOW TO PLAY:
- Start from the top-left corner (marked with >)
- Choose a color/shape to flood adjacent matching tiles
- Each move spreads your chosen color from the flooded area
- You have limited moves to complete the board
- Plan strategically to maximize your flooding efficiency!

TIP: Look for colors that will connect separate regions of your current area.
''')
...
def displayBoard(board, displayMode):
    bext.clear()
    bext.fg('white')
    print(DOWNRIGHT + (LEFTRIGHT * BOARD_WIDTH * 2) + DOWNLEFT)
    ...
    for y in range(BOARD_HEIGHT):
        ...
        for x in range(BOARD_WIDTH):
            ...
            if displayMode == COLOR_MODE:
                ...
                print(SHAPES_MAP[board[(x, y)]] + SHAPES_MAP[board[(x, y)]], end='')

        bext.fg('white')
        print(UPDOWN)
    print(UPRIGHT + (LEFTRIGHT * BOARD_WIDTH * 2) + UPLEFT)
...
def askForPlayerMove(displayMode):
    while True:
        ...
        if response == 'QUIT':
            ...
            sys.exit()
        if displayMode == COLOR_MODE and response in tuple('RGWYBP'):
            return {'R': 0, 'G': 1, 'W': 2, 'Y': 3, 'B': 4, 'P': 5}[response]
